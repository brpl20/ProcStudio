...
describe ProfileCustomers::CreateDocumentService do
  describe '.call' do
    ...
    let!(:document) { create(:customer_file, profile_customer: profile_customer_one, file_description: 'simple_procuration') }
    ...
    context 'when profile_customer do not have documents' do
      it 'do not calls create a document service' do
        ...
        expect(ProfileCustomers::SimpleProcurationService).not_to have_received(:call).with(document.id, profile_admin.id)
      end
    end
  end
end
