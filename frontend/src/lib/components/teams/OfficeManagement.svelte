<script lang="ts">
  ...
  import OfficeDetailsModal from '../../pages/Office/OfficeDetailModal.svelte';
  ...
  let selectedOfficeForDetails = $state(null);
  ...
  function handleShowDetails(office) {
    selectedOfficeForDetails = office;
  }

  function handleCloseDetails() {
    selectedOfficeForDetails = null;
  }
    ...
    if (
      window.confirm(
        `Tem certeza que deseja ${
          office.deleted ? 'excluir permanentemente' : 'arquivar'
        } o escritório "${office.name}"?`
      )
    ) {
      try {
...
<div class="w-full px-4 sm:px-6 lg:px-8 py-8 space-y-6">
  {#if selectedOfficeForDetails}
    <OfficeDetailsModal office={selectedOfficeForDetails} onClose={handleCloseDetails} />
  {/if}
  ...
  {#if showOfficeSelection}
    ...
    >
      <div
        class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl mx-4 animate-in fade-in slide-in-from-bottom-4 duration-300"
      >
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            >
              <div class="flex flex-col items-center text-center space-y-4">
                <div
                  class="w-20 h-20 rounded-2xl bg-gradient-to-br from-[#eef0ef] to-[#0277EE]/10 flex items-center justify-center group-hover:scale-110 transition-transform duration-300"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="h-10 w-10 text-[#0277EE]"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                ...
                <div>
                  ...
                  <p class="text-sm text-gray-600">Cadastre um escritório seu que já existe</p>
                </div>
                <div class="w-full pt-4">
                  <div
                    class="px-6 py-3 rounded-xl bg-gradient-to-r from-[#0277EE] to-[#01013D] text-white font-semibold text-sm group-hover:shadow-lg group-hover:shadow-[#0277EE]/30 transition-all duration-300"
                  >
                    Selecionar
                  </div>
                </div>
  ...
  {:else if showForm}
    <OfficeForm {editingOffice} on:close={handleCloseForm} on:success={handleFormSuccess} />
  {:else}
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-6">
      <div>
        <h2
          class="text-3xl sm:text-4xl font-bold bg-gradient-to-r from-[#01013D] to-[#01013D] bg-clip-text text-transparent mb-2"
        >
    ...
    <div class="flex items-center gap-4">
      <label class="flex items-center gap-3 cursor-pointer group">
        <div class="relative">
          <input type="checkbox" class="sr-only peer" bind:checked={showDeleted} onchange={toggleShowDeleted} />
          <div
            class="w-11 h-6 bg-gray-200 rounded-full peer-checked:bg-gradient-to-r peer-checked:from-[#0277EE] peer-checked:to-[#01013D] transition-all duration-300"
          />
          <div
            class="absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition-transform duration-300 peer-checked:translate-x-5 shadow-md"
          />
        </div>
        <span
          class="text-sm font-medium text-gray-700 group-hover:text-[#0277EE] transition-colors duration-200"
        >
          Mostrar arquivados
        </span>
      </label>
    ...
    {#if !hasActiveLawyers}
      <div
        class="bg-yellow-50 border-l-4 border-yellow-500 rounded-lg p-4 flex items-start gap-3 animate-in fade-in slide-in-from-top-2 duration-300"
      >
        ...
        <div>
          ...
          <p class="text-yellow-800 text-sm mt-1">
            Não é possível criar um escritório sem advogados ativos no sistema. Por favor, cadastre e
            ative pelo menos um advogado antes de criar um escritório.
          </p>
        </div>
      </div>
    {/if}
    ...
    {#if error}
      <div
        class="bg-red-50 border-l-4 border-red-500 rounded-lg p-4 flex items-start gap-3 animate-in fade-in slide-in-from-top-2 duration-300"
      >
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-[#eef0ef] overflow-hidden">
      {#if loading}
        <div class="p-12 text-center">
          <div class="inline-flex items-center justify-center">
            <div
              class="w-12 h-12 border-4 border-[#eef0ef] border-t-[#0277EE] rounded-full animate-spin"
            />
          </div>
      ...
      {:else if offices.length === 0}
        <div class="p-12 text-center">
          <div
            class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-[#eef0ef] to-[#0277EE]/10 mb-4"
          >
