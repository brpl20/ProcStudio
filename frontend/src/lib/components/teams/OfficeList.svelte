<script lang="ts">
  ...
  let {
    offices = [],
    showDeleted = false,
    onEdit = (_office: Office) => {},
    onDelete = (_office: Office) => {},
    onRestore = (_office: Office) => {},
    onShowDetails = (_office: Office) => {}
  }: {
    offices?: Office[];
    showDeleted?: boolean;
    ...
    onShowDetails?: (office: Office) => void;
  } = $props();
...
<div class="space-y-4">
  ...
  {:else}
    <div class="overflow-x-auto">
      <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-1">
        {#each offices as office (office.id)}
          <div
            class="bg-white rounded-2xl border border-[#eef0ef] hover:border-[#0277EE] hover:shadow-lg hover:shadow-[#0277EE]/10 transition-all duration-300 overflow-hidden group {office.deleted
              ? 'opacity-60'
              : ''} cursor-pointer"
            onclick={() => onShowDetails(office)}
            role="button"
            tabindex="0"
            onkeypress={(e) => e.key === 'Enter' && onShowDetails(office)}
          >
            <div class="p-6">
              <div class="flex items-start justify-between mb-4">
                <div class="flex items-start gap-4 flex-1">
                  ...
                  <div class="flex-1 min-w-0">
                    <h3
                      class="font-bold text-lg text-[#01013D] {office.deleted
                        ? 'line-through'
                        : ''} truncate"
                    >
                      {office.name}
                    ...
                    <div class="flex items-center gap-2 mt-2">
                      {#if office.deleted}
                        <span
                          class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-red-100 text-red-900 text-xs font-semibold"
                        >
                      ...
                      {:else}
                        <span
                          class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-green-100 text-green-900 text-xs font-semibold"
                        >
                ...
                <div class="relative group/menu ml-4" onclick={(event) => event.stopPropagation()}>
                  <button
                  ...
                  <div
                    class="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-[#eef0ef] opacity-0 invisible group-hover/menu:opacity-100 group-hover/menu:visible transition-all duration-200 z-50"
                  >
                    {#if !office.deleted}
                      <button
              ...
              <div class="grid grid-cols-2 md:grid-cols-3 gap-4 pt-4 border-t border-[#eef0ef]">
                <div>
                  ...
                  <p class="text-sm font-mono font-semibold text-[#01013D]">
                    {office.cnpj || '-'}
                  </p>
                </div>
                <div>
    ...
    <div
      class="mt-6 p-4 bg-gradient-to-r from-[#eef0ef] to-[#0277EE]/5 rounded-xl border border-[#eef0ef]"
    >
