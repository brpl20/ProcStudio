...
RSpec.describe Api::V1::ZapsignController, type: :request do
  ...
  before do
    ...
    allow(CredentialsHelper).to receive(:get).with(:zapsign, :webhook_secret_key).and_return('valid_secret_key')
  end
  ...
  describe 'POST #create' do
    context 'com documentos válidos' do
      ...
      it 'atualiza o status do documento para pending_external_signature e retorna sucesso' do
        post '/api/v1/zapsign', params: { work_id: work.id },
  ...
  describe 'POST #webhook' do
    ...
    context 'quando o secret_key é válido' do
      ...
      before do
        allow(zapsign_service).to receive(:receive_signed_doc)
                                    .and_return({ message: 'Documento atualizado para signed.' })
      end

      it 'atualiza o status do documento para signed' do
        post '/api/v1/zapsign/webhook', params: payload.to_json,
                                        headers: {
                                          'HTTP_AUTHORIZATION' => 'valid_secret_key',
    ...
    context 'quando o secret_key é inválido' do
      ...
      it 'retorna erro de acesso não autorizado' do
        post '/api/v1/zapsign/webhook', params: payload.to_json,
                                        headers: {
                                          'HTTP_AUTHORIZATION' => 'invalid_secret_key',
                                          'Content-Type' => 'application/json'
                                        }
