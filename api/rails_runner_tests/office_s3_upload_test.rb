...
class OfficeS3UploadTest
  ...
  def check_environment_vars
    ...
    bucket_var = ENV['S3_BUCKET'] || ENV['AWS_BUCKET_MAIN']
    if bucket_var.blank?
        ...
        if contract_metadata
          ...
          if verify_s3_file_exists(contract_metadata.s3_key)
            ...
            if url
              ...
              if download_url
                puts "   ✅ Generated download URL with Content-Disposition header"
              end
            else
              puts "   ⚠️  Could not generate presigned URL"
            end
    ...
    begin
      ...
      if @test_office
        ...
        @test_office.attachment_metadata.each do |metadata|
          if metadata.s3_key.present? && !metadata.s3_key.start_with?('local/')
            delete_s3_file(metadata.s3_key)
          end
        end
  ...
  def create_test_image_file(filename)
    file = Tempfile.new([filename.gsub('.', '_'), '.png'])
  ...
  def create_test_pdf_file(filename)
    file = Tempfile.new([filename.gsub('.', '_'), '.pdf'])
  ...
  def verify_s3_file_exists(s3_key)
    ...
    s3_client.head_object(bucket: ENV['S3_BUCKET'], key: s3_key)
    true
  ...
  def delete_s3_file(s3_key)
    ...
    s3_client.delete_object(bucket: ENV['S3_BUCKET'], key: s3_key)
