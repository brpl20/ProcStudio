/**
 * Generated API Tests for Development/Customer/Works
...
describe('Development/Customer/Works', function() {
  this.timeout(30000);
  ...
  before(async function() {
    ...
    console.log(`🔐 Authenticating for Development/Customer/Works tests...`);
    ...
    if (config.api.auth && config.api.auth.testCredentials) {
      try {
        ...
        console.log(`✅ Authentication successful for Development/Customer/Works`);
      } catch (error) {
        console.error(`❌ Authentication failed for Development/Customer/Works:`, error.message);
        if (error.response) {
  ...
  it('Development/Customer/Works - Listar Registros', async function() {
    ...
    try {
      ...
      console.log(`✅ ${response.status} - Development/Customer/Works - Listar Registros`);

    } catch (error) {
      if (error.response) {
        console.error(`❌ ${error.response.status} - Development/Customer/Works - Listar Registros:`, error.response.data);
        throw new Error(`Request failed with status ${error.response.status}: ${JSON.stringify(error.response.data)}`);
      } else {
        console.error(`❌ Network error - Development/Customer/Works - Listar Registros:`, error.message);
        throw error;
      }
    }
  });

  it('Development/Customer/Works - Consultar um registro', async function() {
    const headers = { ...config.api.headers, ...getAuthHeaders(), {
    "Authorization": "Bearer eyJhbGciOiJIUzI1NiJ9.eyJhZG1pbl9pZCI6MSwiZXhwIjoxNjkxNzA5NDY5fQ.WV-uHlDdeK2S1tRKR4ddLpKZJnPlsyObmdtKUrJUOMA"
} };
    ...
    try {
      ...
      console.log(`✅ ${response.status} - Development/Customer/Works - Consultar um registro`);

    } catch (error) {
      if (error.response) {
        console.error(`❌ ${error.response.status} - Development/Customer/Works - Consultar um registro:`, error.response.data);
        throw new Error(`Request failed with status ${error.response.status}: ${JSON.stringify(error.response.data)}`);
      } else {
        console.error(`❌ Network error - Development/Customer/Works - Consultar um registro:`, error.message);
        throw error;
      }
    }
  });
